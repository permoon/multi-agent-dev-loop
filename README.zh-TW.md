...
| 3.5. Red team，條件式 | Claude + Codex | `plans/<feature>/red-team.md` + 修訂後的 `validation.md`（Pass 2）|
...
| 6. 部署 review，GCP 條件式 | Claude（載入 GCP skills）| `plans/<feature>/review-gcp.md` |
...
- Step 6（GCP deploy review）需要在 Claude 環境安裝 `SKILL.md` Step 6
  所列的 Google Cloud skills（例如 `bigquery-basics`、`cloud-run-basics`、
  `google-cloud-recipe-auth`、`google-cloud-waf-security` 等），讓 review
  時可以載入
...
- 如果 GCP 風險高，由 Claude 載入相關 Google Cloud skills 進行 deploy review
